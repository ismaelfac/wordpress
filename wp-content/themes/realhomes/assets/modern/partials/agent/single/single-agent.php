...
<section class="rh_section rh_section--flex rh_wrap--padding rh_wrap--topPadding">
	...
	<div class="rh_page rh_page__agents rh_page__main">
		...
		<?php if ( empty( $header_variation ) || ( 'none' === $header_variation ) ) : ?>
			<div class="rh_page__head">
				...
				<h2 class="rh_page__title">
					<p class="sub"><?php esc_html_e( 'Agent', 'framework' ); ?></p>
				    <p class="title"><?php esc_html_e( 'Profile', 'framework' ); ?></p>
				</h2>
				<!-- /.rh_page__title -->
		...
		if ( have_posts() ) :
			while ( have_posts() ) :
				the_post();
				get_template_part( 'assets/modern/partials/agent/single/single', 'agent-card' );
			endwhile;
		endif;
		?>
