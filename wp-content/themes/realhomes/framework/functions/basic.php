...
if ( ! function_exists( 'admin_js' ) ) {
	...
	function admin_js( $hook ) {
		...
		wp_register_script( 'admin-script-js', INSPIRY_COMMON_URI . 'js/admin-script.js', array( 'jquery' ), INSPIRY_THEME_VERSION );
...
if ( ! function_exists( 'display_notice' ) ) {
	...
	function display_notice( $notices = array() ) {
		...
		if ( ! is_array( $notices ) || empty( $notices ) ) return false;
...
if ( ! function_exists( 'google_map_needed' ) ) {
	...
	function google_map_needed() {
		if ( is_page_template( 'templates/template-contact.php' ) && ( get_option( 'theme_show_contact_map' ) == 'true' ) ) {
			return true;
		} elseif ( is_page_template( 'templates/template-map-based-listing.php' ) || is_page_template( 'templates/template-submit-property.php' ) ) {
			return true;
		...
		} elseif ( is_page_template( 'templates/template-search.php' ) && ( get_option( 'theme_search_module' ) == 'properties-map' ) ) {
			return true;
		} elseif ( ( is_page_template( 'templates/template-search-sidebar.php' ) || is_page_template( 'templates/template-search-right-sidebar.php' ) ) && ( get_option( 'theme_search_module' ) == 'properties-map' ) ) {
			return true;
		} elseif ( is_page_template( 'templates/template-property-listing.php' ) || is_page_template( 'templates/template-property-grid-listing.php' ) || is_tax( 'property-city' ) || is_tax( 'property-status' ) || is_tax( 'property-type' ) || is_tax( 'property-feature' ) || is_post_type_archive( 'property' ) ) {
...
if ( ! function_exists( 'inspiry_get_listed_properties' ) ) {
	 ...
	 * @since 3.0.0
	 */
	function inspiry_get_listed_properties( $agent_id ) {
